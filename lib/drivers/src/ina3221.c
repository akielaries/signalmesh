...
static bool ina3221_read_all_helper(ina3221_t *dev_data,
...
static int ina3221_remove(device_t *dev);
static int ina3221_ioctl(device_t *dev, uint32_t cmd, void *arg);
...
// INA3221 specific reading channels
...
static int ina3221_init(device_t *dev) {
  ...
  // Build configuration register value using macros from ina3221.h
  uint16_t config_value = INA3221_CONFIG_BVOLTAGERANGE_32V | INA3221_CONFIG_GAIN_8_320MV |
                          (INA3221_CONFIG_ADC_12_BIT_1_SAMPLES << INA3221_CONFIG_BADCRES_SHIFT) |
                          (INA3221_CONFIG_ADC_12_BIT_1_SAMPLES << INA3221_CONFIG_SADCRES_SHIFT) |
                          INA3221_CONFIG_MODE_SANDBVOLT_CONT;
  ...
  bsp_printf("INA3221: Current LSB: %.6f A/bit (%.3f mA/bit)\n",
             ina_dev->current_lsb,
             ina_dev->current_lsb * 1000.0f);
  bsp_printf("INA3221: Power LSB: %.6f W/bit (%.3f mW/bit)\n",
             ina_dev->power_lsb,
             ina_dev->power_lsb * 1000.0f);
  bsp_printf("INA3221: Writing calibration value: %d\n", ina_dev->calibration_value);

  if (ina3221_write_register(ina_dev, INA3221_REG_CALIBRATION, ina_dev->calibration_value) != 0) {
    bsp_printf("INA3221: Failed to write calibration.\n");
    return DRIVER_ERROR;
  }
  ...
  if (ina3221_read_register(ina_dev, INA3221_REG_CONFIG, &read_config) == 0) {
    bsp_printf("INA3221: Read back config: 0x%04X\n", read_config);
  } else {
    bsp_printf("INA3221: Failed to read back config.\n");
    return DRIVER_ERROR;
  }

  uint16_t read_cal;
  if (ina3221_read_register(ina_dev, INA3221_REG_CALIBRATION, &read_cal) == 0) {
    bsp_printf("INA3221: Read back calibration: %d\n", read_cal);
  }
...
static int ina3221_poll(device_t *dev, uint32_t num_readings, driver_reading_t *readings) {
  ...
  if (num_readings > ina3221_readings_directory.num_readings) {
    return DRIVER_INVALID_PARAM;
  }

  ina3221_t *ina_dev_data = (ina3221_t *)dev->priv;
  float shunt_v, bus_v, current_ma, power_mw;

  if (!ina3221_read_all_helper(ina_dev_data, &shunt_v, &bus_v, &current_ma, &power_mw)) {
    return DRIVER_ERROR;
  }

  // Populate readings array
  for (uint32_t i = 0; i < num_readings; ++i) {
    const driver_reading_channel_t *channel = &ina3221_readings_directory.channels[i];
...
static bool ina3221_read_shunt_voltage_helper(ina3221_t *dev_data, float *shunt_voltage_mv) {
  uint16_t raw_value;
  if (ina3221_read_register(dev_data, INA3221_REG_SHUNTVOLTAGE, &raw_value) != 0) {
    return false;
  }
...
static bool ina3221_read_bus_voltage_helper(ina3221_t *dev_data, float *bus_voltage_v) {
  uint16_t raw_value;
  if (ina3221_read_register(dev_data, INA3221_REG_BUSVOLTAGE, &raw_value) != 0) {
    return false;
  }
...
static bool ina3221_read_current_helper(ina3221_t *dev_data, float *current_ma) {
  uint16_t raw_value;
  if (ina3221_read_register(dev_data, INA3221_REG_CURRENT, &raw_value) != 0) {
    return false;
  }
...
static bool ina3221_read_power_helper(ina3221_t *dev_data, float *power_mw) {
  uint16_t raw_value;
  if (ina3221_read_register(dev_data, INA3221_REG_POWER, &raw_value) != 0) {
    return false;
  }
...
static bool ina3221_read_all_helper(ina3221_t *dev_data,
                                   ...
                                   float *current_ma,
                                   float *power_mw) {
  return ina3221_read_shunt_voltage_helper(dev_data, shunt_voltage_mv) &&
         ina3221_read_bus_voltage_helper(dev_data, bus_voltage_v) &&
         ina3221_read_current_helper(dev_data, current_ma) &&
         ina3221_read_power_helper(dev_data, power_mw);
}
