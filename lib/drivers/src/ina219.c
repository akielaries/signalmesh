...
static bool ina219_write_register_helper(ina219_t *dev_data, uint8_t reg, uint16_t value);
static bool ina219_read_register_helper(ina219_t *dev_data, uint8_t reg, uint16_t *value);
...
static int ina219_poll_fn(device_t *dev) {
    ...
    if (ina219_read_all_helper(ina_dev_data, &shunt_v, &bus_v, &current_ma, &power_mw)) {
        bsp_printf("INA219 (%s) - Shunt: %.2fmV, Bus: %.2fV, Current: %.2fmA, Power: %.2fmW\n",
                   dev->name, shunt_v, bus_v, current_ma, power_mw);
        return DRIVER_OK;
    }
    return DRIVER_ERROR;
}
...
static bool ina219_reset_helper(ina219_t *dev_data) {
  return ina219_write_register_helper(dev_data, INA219_REG_CONFIG, INA219_CONFIG_RESET);
}
...
static bool ina219_configure_helper(ina219_t *dev_data, const ina219_config_t *config) {
  ...
  // Write configuration
  bool success = ina219_write_register_helper(dev_data, INA219_REG_CONFIG, config_value);
  if (!success) {
    return false;
  }

  // Write calibration register
  return ina219_write_register_helper(
                               dev_data,
                               INA219_REG_CALIBRATION,
                               dev_data->calibration_value);
}
...
static bool ina219_read_shunt_voltage_helper(ina219_t *dev_data, float *shunt_voltage_mv) {
  uint16_t raw_value;
  if (!ina219_read_register_helper(dev_data, INA219_REG_SHUNTVOLTAGE, &raw_value)) {
    return false;
  }
...
static bool ina219_read_bus_voltage_helper(ina219_t *dev_data, float *bus_voltage_v) {
  uint16_t raw_value;
  if (!ina219_read_register_helper(dev_data, INA219_REG_BUSVOLTAGE, &raw_value)) {
    return false;
  }
