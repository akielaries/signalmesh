...
void ddr3_rw_test(void) {
  ...
  static uint32_t write_data[6][4] = {{0x91234567, 0x89abcdef, 0xfedcba98, 0x76543210},
  ...
  for (int i = 0; i < 6; i++) {
    ...
    // Check if correct
    if (read_data[i][0] == write_data[i][0] && read_data[i][1] == write_data[i][1] &&
        read_data[i][2] == write_data[i][2] && read_data[i][3] == write_data[i][3]) {
      dbg_printf("OK\r\n");
    } else {
      dbg_printf("MISMATCH\r\n");
    }
