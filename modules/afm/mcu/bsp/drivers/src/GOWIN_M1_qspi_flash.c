...
void qspi_flash_init(void)
{
	...
	SPI_FLASH->TIMING |= (0x4);        //Set the QSPI Interface Timing Register (0x40)
...
void qspi_flash_io_fast_read(uint16_t rd_len, uint32_t cmd, uint32_t address,uint8_t *read_buffer)
{
	...
	{	
		//check the status of RX FIFO
		while(SPI_FLASH->STATUS & (0x1 << 14));
