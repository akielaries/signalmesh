...
#include "ch.h"
#include "hal.h"
#include "bsp/utils/bsp_io.h" // For printing utility
...
static float adc_to_vsense(uint16_t ts_data) {
  ...
  float temp = ts_cal1_temp + ((float)(ts_data_12bit - ts_cal1)) *
...
void run_test_adc(void) {
  ...
  bsp_printf("--- Starting ADC Test ---\n");
  bsp_printf("Reading potentiometers, thermistor, and internal sensors.\n");
  ...
  while (true) {
    ...
    // Extract individual values (based on how they are defined in
    // bsp_adc_config.c)
    ...
    // Print values
    bsp_printf("Pot1: %d  Pot2: %d Pot3: %d \n", pot1, pot2, pot3);
    bsp_printf("Therm1: %d (%f C) V_SENSE: %d (%f C)\n\n",
               therm1,
               adc_to_steinhart(therm1),
               v_sense,
               adc_to_vsense(v_sense));
    bsp_printf("VREF: %d VREF_CAL: %d VDDA: %f V\n",
               v_ref,
               *VREFINT_CAL_ADDR,
               adc_to_vdda(v_ref));
    ...
    if (palReadLine(PORTAB_LINE_BUTTON) == PORTAB_BUTTON_PRESSED) {
      ...
      bsp_printf("ADC conversions stopped.\n");
      break;
    }
